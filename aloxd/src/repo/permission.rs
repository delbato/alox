...
impl PermissionRepo {
    ...
    pub async fn find_for_user(&self, user_key: &String) -> Result<Vec<Permission>, ()> {
        self.database.aql_bind_vars("
            FOR perm IN permissions
                FILTER perm._key_user == @user_key
                RETURN perm
        ", hashmap!{ "user_key" => json!(user_key) }).await
            .map_err(|_| ())
    }
}
